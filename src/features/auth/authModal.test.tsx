...
describe('AuthModal (PRD §7.1)', () => {
  ...
  it('shows one generic error for invalid login (AUTH-08)', async () => {
    const logIn = vi
      .fn()
      .mockResolvedValue({ ok: false, error: { code: 'AUTH_INVALID', message: 'Invalid username or password.' } })
    ...
    expect(await screen.findByTestId('auth-error')).toHaveTextContent(/Invalid username or password/i)
...
describe('authStore', () => {
  ...
  it('initializes unauthenticated when no session exists (AUTH-09)', async () => {
    mockGetAuthService.mockReturnValue(makeService({ restoreSession: vi.fn().mockResolvedValue(null) }))
  ...
  it('register flow authenticates on success and reports failure', async () => {
    mockGetAuthService.mockReturnValue(makeService({ signUp: vi.fn().mockResolvedValue(ok(session)) }))
