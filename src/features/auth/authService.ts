...
function toAuthError(error: unknown): AuthError {
  ...
  if (
    isAuthErrorWithStatus(error, 422) ||
...
export class AuthService {
  ...
  async signUp(rawUsername: string, password: string): Promise<AuthResult<AuthSession>> {
    ...
    if (!usernameCheck.valid) {
      return {
        ok: false,
        error: { code: 'USERNAME_INVALID', message: 'Invalid username format.' },
      };
    }
    ...
    if (!passwordCheck.valid) {
      return {
        ok: false,
        error: { code: 'PASSWORD_INVALID', message: 'Invalid password format.' },
      };
    }
    ...
    if (!data.session || !data.user) {
      return {
        ...
        error: {
          code: 'UNKNOWN',
          message: 'Account created but the session is unavailable. Try logging in.',
        },
      };
    }
  ...
  async logIn(rawUsername: string, password: string): Promise<AuthResult<AuthSession>> {
    ...
    if (!usernameCheck.valid) {
      return {
        ok: false,
        error: { code: 'USERNAME_INVALID', message: 'Invalid username or password.' },
      };
    }
    const { canonical, trimmed } = normalizeUsername(rawUsername);
    const email = toInternalEmail(canonical);
