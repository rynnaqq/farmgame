...
export type PacketValidationResult =
  { ok: true; packet: MovementPacketV1 } | { ok: false; reason: PacketRejection };
...
export function validateMovementPacket(
  ...
  if (typeof raw !== 'object' || raw === null) return { ok: false, reason: 'MALFORMED_PAYLOAD' };
  ...
  if (
    ...
    !Array.isArray(p.p) ||
    p.p.length !== 3 ||
    !p.p.every((n) => typeof n === 'number' && Number.isFinite(n)) ||
    typeof p.anim !== 'number' ||
    p.anim < 0 ||
    p.anim > 3
  ) {
    return { ok: false, reason: 'NON_FINITE' };
...
export class RemotePlayerBuffer {
  ...
  constructor(
    ...
    private readonly renderDelayMs = REMOTE_RENDER_DELAY_MS,
    private readonly maxExtrapolationMs = MAX_EXTRAPOLATION_MS
  ) {
    // renderDelayMs documents the intended render lag; the renderer computes
    // the sample time from the newest snapshot, so it needs no runtime use here.
    void this.renderDelayMs;
  }
