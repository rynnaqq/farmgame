...
export type FarmPatchListener = (patch: {
  ownerId: string
  plotVersion: number
  tiles: Array<{ i: number; state: number; crop: string | null; plantedAt: string | null; readyAt: string | null; mutation: number }>
}) => void
...
export class RoomConnection {
  ...
  constructor(options: RoomConnectionOptions = {}) {
    ...
    this.getUserId =
      options.getUserId ?? (() => useAuthStore.getState().session?.userId ?? null)
  ...
  async join(): Promise<void> {
    ...
    try {
      ...
      useNetStore.getState().setError(error instanceof Error ? error.message : 'Failed to join room')
  ...
  private handleMovement(raw: unknown): void {
    ...
    const ctx = {
      roomId: this.roomId,
      memberIds: new Set(useNetStore.getState().members.map((m) => m.userId)),
...
function resetNetStoreSafe(): void {
